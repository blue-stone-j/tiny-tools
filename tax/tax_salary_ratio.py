...
# Calculate ratios
ratios = [taxed / cost if cost != 0 else 0 for taxed, cost in zip(taxed_salaries, employee_costs)]
ratios2 = [1 - ratio for ratio in ratios]

# Plot the ratio curve
plt.figure(figsize=(10, 6))
plt.plot(salaries, ratios, label='Ratio (Taxed Salary / Employee Cost)', color='blue')
plt.plot(salaries, ratios2, label='Ratio (Gross tax / Employee Cost)', color='red', linestyle='--')
plt.title('Taxed Salary vs Employee Cost in Shanghai (2023)', fontsize=14)
plt.xlabel('Gross Salary (CNY)', fontsize=12)
plt.ylabel('Ratio (Taxed Salary / Employee Cost)', fontsize=12)
